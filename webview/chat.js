(function(){
  // Electron shim: provide VS Code-compatible API backed by window.electronAPI
  function _acquireVsCodeApi() {
    var api = (window.electronAPI && window.electronAPI.isElectron) ? window.electronAPI : null;
    return {
      postMessage: function(msg) {
        if (api) api.postMessage(msg);
        else console.log('[postMessage]', msg);
      },
      getState: function() {
        if (api) return api.getState();
        return (window._dcState || {});
      },
      setState: function(state) {
        if (api) api.setState(state);
        else window._dcState = state;
      }
    };
  }
  var vscode;
  try { vscode = _acquireVsCodeApi(); } catch(e) { document.body.innerHTML += "<div style=\"color:red;padding:10px\">API error: "+e.message+"</div>"; return; }
  ...
  /* ─── Message handler ──────────────────────────────────────────────── */
  // Electron message listener (replaces window.addEventListener("message", ...))
  function _dispatchMessage(m) {
  ...
  // Wire up message listener — Electron IPC or standard message event
  if (window.electronAPI && window.electronAPI.isElectron) {
    window.electronAPI.onMessage(function(msg) { _dispatchMessage(msg); });
  } else {
    window.addEventListener("message", function(e) { _dispatchMessage(e.data); });
  }
